...
кратко про пункт:
Делаем “текстовый интеллект”:
	•	выбираем модель эмбеддингов (обычно готовая, не обучаем с нуля)
	•	поднимаем/подключаем Qdrant для векторов
	•	делаем извлечение навыков из текста (резюме/вакансии) + нормализацию под Compass Skill
...
кратко про пункт:
Тут начинается обучение первой нейросети (или sequence-модели):
	•	готовим датасет последовательностей ролей
	•	обучаем модель next-role prediction
	•	сохраняем веса/маппинги как “model pack”
	•	добавляем объяснимость (частые переходы/похожие траектории/skill-gap)

тут уже “обучаем нейросеть” по-настоящему.
...
кратко про пункт:
Это обычно не нейросеть, а бустинг:
	•	учим модель по вакансиям hh (или демо-срезу)
	•	выдаём вилки зарплат для TeamPlan и найма
...
крако про пункт:
Собираем цепочку:
	•	skill-gap между профилем сотрудника и ролью
	•	подбор курсов под gap (через эмбеддинги + покрытие навыков)
	•	формируем объяснения “почему этот курс/роль”
...
кратко про пункт:
Алгоритм (обычно не нейросеть):
	•	кого можно назначить сразу
	•	кого выгодно “докачать” (gap+курсы+ETA)
	•	кого надо нанимать (вилка ЗП + ключевые навыки)
...
кратко про пункт:
Упаковываем всё ML в сервис:
	•	/health, /career/predict-next, /skills/extract, /learning/recommend, /team/plan, /market/salary-range
	•	чтобы портал дергал это как внешний “движок”
...
кратко про пункт:
Подключаем кнопки и записываем результаты в твои DocTypes:
	•	кнопка в Employee → вызов ML → запись Compass Career Prediction + Compass Learning Recommendation
	•	кнопка TeamPlan → вызов ML/логики → запись результатов
	•	и тут же строгие правила видимости (row-level access)
...
кратко про пункт:
Поддержка PDF/DOCX/TXT: (предположительно из резюме с ХХ ру)
	•	распарсили → извлекли навыки → обновили Compass Employee Skill Profile
	•	кнопка “Refresh Skills from Resume” начинает работать по-настоящему
...
Тесты на:
	...
	•	извлечение навыков
	•	модели
	•	интеграцию (кнопка → запись в DocTypes)
	•	smoke-тест “всё поднялось и отдает ответы”
...
кратко про пункт:
Минимум доков: архитектура, API контракт, источники данных, model cards, демо-сценарии.
...
кратко про пункт:
Делаем нормальную “доставку”:
	•	версии, model packs отдельно от git
	•	demo-up, demo-seed, download models, smoke_test скрипты
...
кратко про пункт: 
Два режима установки (как app / demo stack), что модуль не ломает HRMS, как подключить model pack и т.д.
